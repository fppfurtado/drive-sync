...
- **Error logs are tail-truncated**: `sync_engine.py:241` records `err.strip()[-500:]`. Cryptic fragments like `"xist?"` are tail-only — real cause is earlier in stderr. Read full log lines, or capture stderr separately when reproducing.
