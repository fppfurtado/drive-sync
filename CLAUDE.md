...
- **bisync errors do NOT auto-recover** (com UMA exceção restrita — ver abaixo): erro genérico de bisync loga e segue; recovery manual `rclone bisync ... --resync` é o caminho. **Recovery playbook** (rc=7 stale-listings + rc=1 too-many-deletes, data-safe order): [docs/operations/playbook-bisync-recovery.md](docs/operations/playbook-bisync-recovery.md) — prefer deleting the per-pair marker to force a live-flags `--resync` over transcribing flags by hand. **Pré-check antes de recuperar: `drive-sync --dry-run-resync <folder>`** (#94) — roda o dry-run com os flags exatos do daemon (`RcloneEngine.build_bisync_cmd`, fonte única) num workdir temporário e classifica a divergência **pelos listings**, não pelo texto do log: `SÓ-ADIÇÕES`/`NO-OP` = data-safe (exit 0) · `TEM OVERWRITE` = destrutivo, decida a direção (exit 1). A distinção não está no log — medido em sandbox (v1.74.3), `--resync` **nunca deleta** (restaura o arquivo do outro lado), e um overwrite de arquivo divergente sai como `Skipped copy`, indistinguível de uma adição inofensiva. **Exceção restrita ([ADR-019](docs/decisions/ADR-019-auto-resync-gated-rc7-stale-listings.md), #47):** um abort **rc=7 stale-listings** (`cannot find prior Path1 or Path2 listings` — estado `.lst` morto, dados intactos) é **auto-recuperado gated-por-prova**: `bisync_folder` roda `--resync --dry-run`, e **só** se a saída provar que a união é no-op (0 transfers / 0 deletes — `_dryrun_resync_is_noop`) reconstrói o baseline com o `--resync` real; divergência real ou dry-run ambíguo → NÃO age, permanece degradado (fail-safe, preserva o espírito do invariante para o caso perigoso). 1-tentativa/episódio — o guard é consumido só por veredito de **divergência**; uma falha de **execução** transitória do `--resync` real (rc≠0, ex.: `connection refused` não classificado como auth) **libera** o guard p/ re-tentar no próximo ciclo ([emenda ADR-019 2026-09-08, #86](docs/decisions/ADR-019-auto-resync-gated-rc7-stale-listings.md) — antes latcheava até restart). Tag `[BISYNC_AUTORESYNC]`; kill-switch `rclone.auto_resync_stale_listings` (default true). OUTRAS causas de rc=7 (case-duplicates ADR-011) e rc=1 too-many-deletes seguem manuais — mas o abort **rc=1 too-many-deletes** emite advice safe tagueado `[BISYNC_SAFETY_ABORT]` (#52): em vez de deixar só a dica `--force` do rclone (que propaga as deleções → perda de dados), loga um contra-advice apontando pro branch rc=1 do playbook (sinalização, **não** recuperação — divergência real segue manual, invariante preservado). **Terceira exceção restrita, COMPOSTA com a de rc=7 ([ADR-022](docs/decisions/ADR-022-orphaned-lock-max-lock.md), #88):** um abort **rc=1 `prior lock file found`** (lock `.lck` órfão — dono do bisync morreu sem liberá-lo: restart/kill mid-bisync, incl. o kill de ADR-018) deixa de travar o folder indefinidamente. O daemon passa `rclone --max-lock <dur>` (knob `rclone.max_lock_seconds`, default 3600; 0=off) em toda invocação de bisync — **prevenção write-time**: grava `TimeExpires=now+dur` e renova a cada dur/2 (run vivo protegido), expirando o lock após a janela quando o dono morre. **NÃO é recuperador direto:** ultrapassar o lock expirado deleta as listings → **rc=7 stale-listings**, que o auto-resync de ADR-019 então recupera (data-safe) — logo há **dependência dura de `auto_resync_stale_listings=true`**. A expiração é pelo `TimeExpires` GRAVADO, não pelo flag do leitor → locks **never-expire pré-existentes** exigem **migração one-time** (`rm` manual, ver ADR-022 § Migração). Investigação: `journalctl --user -u drive-sync --grep "BISYNC_AUTORESYNC"` (auto-resync rc=7) · `--grep "BISYNC_SAFETY_ABORT"` (advice rc=1).
...
- **Órfãos de cobertura são avisados (warn, não fatal) config-time** ([ADR-015](docs/decisions/ADR-015-audit-cobertura-orfaos-config-time.md) + [ADR-021](docs/decisions/ADR-021-coverage-audit-model-a-roots-glob-allow.md)): `drive-sync --check` roda `audit_coverage_orphans` (em `config.py`) — sinaliza filhos-diretório **com conteúdo** que nenhum folder cobre e não estão em `coverage_audit.allow` ("órfãos de cobertura" — conteúdo local não backup-eado). O universo-a-escanear é a **UNIÃO de dois modelos** (ADR-021, #77): **Modelo B** (pais dos `local_path` configurados — #56) **∪** **Modelo A** (`coverage_audit.roots` declaradas), este fechando o ponto-cego de top-level **sem sibling configurado** — o caso `~/mneme` filho-direto de `$HOME` que nenhum `local_path` tornava visível (#74; gate #55 disparou). **Classe INVERSA de ADR-010/011**: aqueles validam malformação DENTRO de um folder declarado (fatal, causa rc=7); este pega conteúdo **não-declarado** (warn — omissão não quebra sync algum). `git_handling` é **ortogonal** (todo declarado é "conhecido", inclusive `bundle`/`skip`). Sinal, não ação: operador cobre (novo `folders:`) ou exclui (`coverage_audit.allow`). **`allow` aceita path LITERAL (casa exato/subpath) E GLOB** (`*?[`, via fnmatch — ADR-021): o glob é o que torna `$HOME`-como-root usável (silencia os dezenas de dotdirs de env-stack — `~/.*`, `~/.local/share/*` — sem enumerá-los), convergindo com os `exclude:` do #55/ADR-020. `roots: []` default = comportamento #56 puro (backward-compat). Opt-out total via `coverage_audit.enabled: false`. Só config-time (arm runtime born-after-config deferido, irmão do #37). Motivação: `pictures/Screenshots` vivo fora do backup por ~3 meses (#54) + `~/mneme` invisível ao audit (#74).
